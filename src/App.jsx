...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    // Crash হলে কুকি ও স্টোরেজ ক্লিয়ার
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    // 3. Inject Heavy-Duty CSS to HIDE everything from Google
    ...
    style.innerHTML = `
      ...
      .translated-mode .judgment-content {
          font-family: 'Kalpurush', 'Hind Siliguri', 'SolaimanLipi', sans-serif !important;
  ...
  const fetchGlobalNotifications = async () => {
    ...
    } catch (e) { console.log('No notification table'); }
  };
  ...
  const handleEditClick = (notif) => {
      ...
      if (notif.expires_at) {
        const d = new Date(notif.expires_at); d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
        setAdminExpiryInput(d.toISOString().slice(0, 16));
      } else { setAdminExpiryInput(''); }
  };
  ...
  const forceSignOut = async ({ title, message }) => {
    ...
    openNotice({ type: 'warning', title: title || 'Signed Out', message: message || 'Session ended.', primaryText: 'OK', onPrimary: closeNotice });
  };
  ...
  useEffect(() => {
    ...
    document.addEventListener('copy', onCopy); document.addEventListener('contextmenu', onContextMenu);
    return () => { document.removeEventListener('copy', onCopy); document.removeEventListener('contextmenu', onContextMenu); };
  }, []);
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          setLoading(false);
          if (data?.session) {
            checkSubscription(data.session.user); updateSessionInDB(data.session);
            if (sessionInterval) clearInterval(sessionInterval); sessionInterval = startSessionMonitor(data.session);
          } else { setSubStatus(false); setProfileData(null); }
        }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (session) {
        ...
        if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') { updateSessionInDB(session); if (sessionInterval) clearInterval(sessionInterval); sessionInterval = startSessionMonitor(session); }
      } else { setSubStatus(false); setProfileData(null); if (sessionInterval) clearInterval(sessionInterval); }
    });
  ...
  const updateSessionInDB = async (currentSession) => {
    ...
    try {
      await supabase.from('members').update({ current_session_id: token }).eq('id', user.id);
      if (user.email) await supabase.from('members').update({ current_session_id: token }).eq('email', user.email);
    } catch (err) { }
  };
  ...
  // --- Reset Language & View when leaving ---
  ...
  const handleSearch = async (page = 1, type = 'simple', termOverride = null) => {
    ...
    if(isTranslated) toggleLanguage(); // Reset translation on search
    try {
  ...
  const loadJudgment = async (item) => {
    ...
    if(isTranslated) toggleLanguage(); // Ensure fresh start
    try {
  ...
  const handleAuth = async (val, pass, isSign) => {
    ...
    if (isSign) {
      ...
      if (error) openNotice({ type: 'error', title: 'Failed', message: error.message });
          ...
          try{ await supabase.from('members').insert([{ id: data.user.id, email: val, expiry_date: null }]); }catch(e){}
          await supabase.auth.signOut(); hardClearAuthStorage(); setModalMode('signupSuccess'); 
      }
    } else {
      ...
      try {
        if (p.type === 'email') res = await supabase.auth.signInWithPassword({ email: p.value, password: pass });
          ...
          const c = normalizePhoneCandidates(p.value);
          for (const ph of c) { res = await supabase.auth.signInWithPassword({ phone: ph, password: pass }); if (!res.error) break; }
        }
      } catch (e) { res = { error: e }; }
  ...
  const handleLogout = async () => { setLoading(true); try { await supabase.auth.signOut(); } catch (e) {} hardClearAuthStorage(); setSession(null); setSubStatus(false); setProfileData(null); setModalMode(null); setLoading(false); window.location.reload(); };
  ...
  const toggleBookmark = async (item) => {
    ...
    if (ex && ex.length > 0) {
      await supabase.from('bookmarks').delete().eq('id', ex[0].id); openNotice({ type: 'info', title: 'Removed', message: 'Bookmark removed.' });
    } else {
      await supabase.from('bookmarks').insert([{ email: id, case_title: item.title, case_citation: item.citation, case_anchor: item.case_anchor, github_filename: item.github_filename }]); openNotice({ type: 'success', title: 'Saved', message: 'Bookmark added.' });
    }
  };
  ...
  return (
    // 'notranslate' class on the outer div might prevent translation of the whole app, 
    // so we apply it carefully to specific sections instead.
    <div>
      {/* Hidden Div for Google Translate Element - Crucial for the engine to work */}
      ...
      {/* Navbar - Added 'notranslate' to prevent translation */}
      ...
      {/* Hero Section - Added 'notranslate' */}
      ...
      <div className="container" style={{ minHeight: '400px' }}>
        ...
        {/* Results Area - Added 'notranslate' */}
        ...
        {/* === READER VIEW (THE ONLY AREA ALLOWED TO TRANSLATE) === */}
        {view === 'reader' && !loading && currentJudgment && (
          // We apply 'translated-mode' class conditionally for font styling
          <div id="readerView" className={`bg-white p-4 p-md-5 rounded-3 shadow-sm border mb-5 ${isTranslated ? 'translated-mode' : ''}`}>
            ...
            {/* THE CONTENT - THIS IS WHAT GETS TRANSLATED */}
        ...
        <div className="modal d-block notranslate" style={{ background: 'rgba(0,0,0,0.8)', zIndex: 1100 }}>
          <div className="modal-dialog modal-lg modal-dialog-scrollable">
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '15px' }}>
              <div className="modal-header bg-dark text-white border-0"><h5 className="modal-title fw-bold"><i className="fas fa-tachometer-alt me-2"></i>Admin Notification Center</h5><button className="btn-close btn-close-white" onClick={() => setModalMode(null)}></button></div>
              <div className="modal-body p-4 bg-light">
                <div className="card border-0 shadow-sm mb-4">
                   <div className="card-header bg-white fw-bold py-3 border-bottom-0 d-flex justify-content-between align-items-center"><span>{isEditing ? 'Edit Notification' : 'Create New Notification'}</span>{isEditing && <button className="btn btn-sm btn-outline-secondary" onClick={cancelEdit}>Cancel Edit</button>}</div>
                   <div className="card-body">
                      <form onSubmit={sendAdminNotification}>
                         <div className="row">
                            <div className="col-md-8 mb-3"><label className="form-label small text-muted text-uppercase fw-bold">Title</label><input type="text" className="form-control" placeholder="Short title" value={adminTitleInput} onChange={e => setAdminTitleInput(e.target.value)} required /></div>
                            <div className="col-md-4 mb-3"><label className="form-label small text-muted text-uppercase fw-bold">Type</label><select className="form-select" value={adminMsgType} onChange={e => setAdminMsgType(e.target.value)}><option value="info">Info (Blue)</option><option value="success">Success (Green)</option><option value="warning">Warning (Yellow/Red)</option></select></div>
                            <div className="col-12 mb-3"><label className="form-label small text-muted text-uppercase fw-bold">Expires At (Optional)</label><input type="datetime-local" className="form-control" value={adminExpiryInput} onChange={e => setAdminExpiryInput(e.target.value)} /><small className="text-muted">Leave blank for no expiration. Users won't see this after this date.</small></div>
                            <div className="col-12 mb-3"><label className="form-label small text-muted text-uppercase fw-bold">Message Content</label><textarea className="form-control" rows="3" placeholder="Write your message here..." value={adminMsgInput} onChange={e => setAdminMsgInput(e.target.value)} required style={{ resize: 'none' }}></textarea></div>
                         </div>
                         <div className="d-flex justify-content-end"><button type="submit" className={`btn px-4 ${isEditing ? 'btn-warning' : 'btn-dark'}`}><i className={`fas ${isEditing ? 'fa-save' : 'fa-paper-plane'} me-2`}></i>{isEditing ? 'Update Notification' : 'Send Broadcast'}</button></div>
                      </form>
                   </div>
                </div>
                <h6 className="fw-bold text-secondary text-uppercase mb-3 small">All Notifications (Including Expired)</h6>
                {globalNotifications.length === 0 ? (<div className="text-center text-muted py-3">No active notifications found.</div>) : (<div className="list-group">{globalNotifications.map((notif) => (<div key={notif.id} className="list-group-item list-group-item-action d-flex justify-content-between align-items-center p-3 border-0 mb-2 shadow-sm rounded"><div className="w-100 pe-3"><div className="d-flex align-items-center mb-1"><span className={`badge me-2 ${notif.type === 'error' ? 'bg-danger' : notif.type === 'warning' ? 'bg-warning text-dark' : 'bg-primary'}`}>{notif.type}</span><h6 className="mb-0 fw-bold">{notif.title}</h6></div><p className="mb-1 text-muted small text-truncate">{notif.message}</p><div className="d-flex gap-3"><small className="text-muted" style={{ fontSize: '10px' }}>Created: {new Date(notif.created_at).toLocaleString()}</small>{notif.expires_at && (<small className={new Date(notif.expires_at) < new Date() ? "text-danger fw-bold" : "text-success fw-bold"} style={{ fontSize: '10px' }}>Expires: {new Date(notif.expires_at).toLocaleString()}</small>)}</div></div><div className="d-flex flex-column gap-2"><button className="btn btn-outline-dark btn-sm rounded-circle" onClick={() => handleEditClick(notif)} title="Edit"><i className="fas fa-edit"></i></button><button className="btn btn-outline-danger btn-sm rounded-circle" onClick={() => deleteNotification(notif.id)} title="Delete"><i className="fas fa-trash"></i></button></div></div>))}</div>)}
              </div>
            </div>
          </div>
        </div>
