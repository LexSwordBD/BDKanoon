...
function AppContent() {
  ...
  const handleSmartEntry = async (e) => {
    ...
    try {
       ...
       console.error(err);
       openNotice({ type: 'error', title: 'Failed', message: err.message || 'Processing failed.' });
    } finally {
  ...
  return (
    <div>
      ...
      {/* Added 'notranslate' to hero section */}
      ...
      <div className="container" style={{ minHeight: '400px' }}>
        ...
        {/* Results View - Added 'notranslate' */}
        ...
        {/* === READER VIEW (The ONLY place where translation is allowed) === */}
        {view === 'reader' && !loading && currentJudgment && (
          // We conditionally add 'translated-mode' for custom Bangla font styling
          <div id="readerView" className={`bg-white p-4 p-md-5 rounded-3 shadow-sm border mb-5 ${isTranslated ? 'translated-mode' : ''}`}>
            ...
            {/* This div DOES NOT have 'notranslate' class, so it gets translated */}
        ...
        {/* Features Section - Added 'notranslate' */}
        {view === 'home' && (
      ...
      {/* --- MODIFIED ADMIN PANEL MODAL (notranslate) --- */}
            ...
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '15px' }}>
              ...
              <div className="modal-body p-0">
                ...
                <div className="d-flex bg-white border-bottom">
                   <button className={`btn flex-fill py-3 rounded-0 fw-bold ${adminTab === 'notifications' ? 'btn-light border-bottom border-3 border-dark' : ''}`} onClick={() => setAdminTab('notifications')}>Notifications</button>
                   <button className={`btn flex-fill py-3 rounded-0 fw-bold ${adminTab === 'dataEntry' ? 'btn-light border-bottom border-3 border-dark' : ''}`} onClick={() => setAdminTab('dataEntry')}>Data Entry</button>
                </div>
                ...
                {adminTab === 'notifications' && (
                  <>
                          ...
                          <form onSubmit={sendAdminNotification}>
                            <div className="row">
                                ...
                                <div className="col-12 mb-3">
                                  ...
                                  <small className="text-muted">Leave blank for no expiration.</small>
                                </div>
                                <div className="col-12 mb-3">
                    ...
                    <h6 className="fw-bold text-secondary text-uppercase mb-3 small">All Notifications</h6>
                            ...
                            <div key={notif.id} className="list-group-item list-group-item-action d-flex justify-content-between align-items-center p-3 border-0 mb-2 shadow-sm rounded">
                                <div className="w-100 pe-3">
                                  ...
                                  <div className="d-flex gap-3">
                                      ...
                                      {notif.expires_at && (<small className={new Date(notif.expires_at) < new Date() ? "text-danger fw-bold" : "text-success fw-bold"} style={{ fontSize: '10px' }}>Expires: {new Date(notif.expires_at).toLocaleString()}</small>)}
                                  </div>
                                </div>
                ...
                {adminTab === 'dataEntry' && (
                   <div className="card border-0 shadow-sm">
                      <div className="card-header bg-white fw-bold py-3 border-bottom-0">
                         <i className="fas fa-robot me-2 text-primary"></i>Smart Judgment Entry
                      </div>
                      <div className="card-body">
                         <form onSubmit={handleSmartEntry}>
                            <div className="row g-3">
                               {/* Auth Token (Secure) */}
                               <div className="col-12">
                                  <label className="form-label small fw-bold text-uppercase">GitHub Access Token</label>
                                  <input type="password" className="form-control" placeholder="ghp_xxxxxxxxxxxx" value={entryToken} onChange={e => setEntryToken(e.target.value)} required />
                               </div>

                               {/* Smart Citation Input */}
                               <div className="col-md-8">
                                  <label className="form-label small fw-bold text-uppercase">Smart Citation Box</label>
                                  <input type="text" className="form-control" placeholder="e.g., 75 DLR (AD) 209, 23 ALR (AD) 400" value={entryCitation} onChange={e => setEntryCitation(e.target.value)} required />
                               ...
                               {/* Title */}
                               <div className="col-12">
                                  <label className="form-label small fw-bold text-uppercase">Case Title (Parties)</label>
                                  <input type="text" className="form-control" placeholder="Abdul vs Rahim" value={entryTitle} onChange={e => setEntryTitle(e.target.value)} required />
                               </div>

                               {/* Headnote */}
                               <div className="col-12">
                                  <label className="form-label small fw-bold text-uppercase">Headnote</label>
                                  <textarea className="form-control" rows="3" placeholder="Write summary/headnote here..." value={entryHeadnote} onChange={e => setEntryHeadnote(e.target.value)} required></textarea>
                               </div>

                               {/* Full Judgment */}
                               <div className="col-12">
                                  <label className="form-label small fw-bold text-uppercase">Full Judgment Text</label>
                                  <textarea className="form-control" rows="6" placeholder="Paste full text here..." value={entryBody} onChange={e => setEntryBody(e.target.value)} required></textarea>
                               </div>

                               <div className="col-12 text-end">
                                  <button type="submit" className="btn btn-dark px-5" disabled={entryLoading}>
                                     {entryLoading ? <><span className="spinner-border spinner-border-sm me-2"></span>Processing...</> : 'Save Data'}
                                  </button>
                               </div>
                            </div>
                         </form>
