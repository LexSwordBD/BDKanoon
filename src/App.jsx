...
export default function App() {
  ...
  const handleSearch = async (page = 1, type = 'simple') => {
    ...
    try {
      ...
      if (data) { setResults(data); setTotalCount(count || 0); }
  ...
  const handleAuth = async (identifierInput, password, isSignUp) => {
    ...
    if (isSignUp) {
      ...
      // Attempt standard Sign Up
      let { data, error } = await supabase.auth.signUp({ email, password });

      // --- START FIX: Handle "Already Registered" for deleted members ---
      // If Supabase says user exists (error or warning), we try to check if they were deleted from DB
      if (error && error.message && (error.message.toLowerCase().includes("registered") || error.message.toLowerCase().includes("already"))) {
        // If Supabase says user exists, we try to sign in with these credentials.
        // This handles the case where Admin deleted the user from 'members' table but Auth user remains.
        const { data: loginData, error: loginError } = await supabase.auth.signInWithPassword({ email, password });
        
        // If login works (password matches), check if the member row is actually missing
        if (!loginError && loginData?.user) {
           const { data: memberExists } = await supabase.from('members').select('id').eq('email', email).maybeSingle();
           
           // If no member row found, we treat this as a valid new signup (Restoration)
           if (!memberExists) {
             error = null;         // Clear the "Already Registered" error
             data = loginData;     // Use the login session data
           }
        }
      }
      // --- END FIX ---
  ...
  const toggleBookmark = async (item) => {
    ...
    const { error } = await supabase.from('bookmarks').insert([{ email: identity, case_title: item.title, case_citation: item.citation, case_anchor: item.case_anchor, github_filename: item.github_filename }]);
